...
describe('WysiwygEditor', () => {
  describe('モード切り替え', () => {
    it('デフォルトでMarkdownモードが選択されていること', () => {
      ...
      const markdownButton = screen.getByRole('button', { name: 'Markdownモード' })
    ...
    it('WYSIWYGモードボタンが表示されること', () => {
      ...
      expect(screen.getByRole('button', { name: 'WYSIWYGモード' })).toBeInTheDocument()
    })

    it('Markdownモードボタンが表示されること', () => {
      renderWithRouter(<NoteEditor onSave={vi.fn()} />)

      expect(screen.getByRole('button', { name: 'Markdownモード' })).toBeInTheDocument()
    })
    ...
    it('WYSIWYGモードに切り替えるとツールバーが表示されること', async () => {
      ...
      expect(screen.getByRole('toolbar', { name: '書式設定ツールバー' })).toBeInTheDocument()
    })
  ...
  describe('ツールバーボタン', () => {
    ...
    it('見出しボタンが表示されること', async () => {
      ...
      expect(screen.getByRole('button', { name: '見出し2' })).toBeInTheDocument()
  ...
  describe('Markdown との相互変換', () => {
    it('Markdownモードで入力した内容がWYSIWYGモードに反映されること', async () => {
      const user = userEvent.setup()
      renderWithRouter(<NoteEditor onSave={vi.fn()} initialContent="**太字テスト**" />)
    ...
    it('WYSIWYGモードからMarkdownモードに戻した時にコンテンツが保持されること', async () => {
      const user = userEvent.setup()
      renderWithRouter(<NoteEditor onSave={vi.fn()} initialContent="テスト本文" />)
