...
test.describe('US2: 新規Markdownノートの作成', () => {
  test('新規ノートを作成して一覧に表示されること', async ({ page }) => {
    ...
    await page.fill('textarea[id="note-content"]', '# テスト\n\nこれはテストノートです。')
...
test.describe('US4: Markdownノートの削除', () => {
  ...
  test('確認ダイアログで削除を実行するとノートが削除されること', async ({ page }) => {
    await fetch('http://localhost:8000/notes', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    await expect(page.locator('h3:has-text("完全削除テスト")')).not.toBeVisible()
  })
...
test.describe('US5: WYSIWYG エディタでの編集', () => {
  test('Markdown モードと WYSIWYG モードを切り替えられること', async ({ page }) => {
    await page.goto('/notes/new')
...
test.describe('US6: リアルタイムプレビュー表示', () => {
  test('Markdown モードでプレビューパネルが表示されること', async ({ page }) => {
    await page.goto('/notes/new')
    // プレビューパネルが表示されている
  ...
  test('プレビューの表示/非表示を切り替えられること', async ({ page }) => {
    ...
    await expect(page.locator('[aria-label="プレビューパネル"]')).not.toBeVisible()
  ...
  test('Markdown 入力がプレビューにリアルタイムで反映されること', async ({ page }) => {
    await page.goto('/notes/new')

    // Markdown を入力
    await page.fill('textarea[id="note-content"]', '# 見出しテスト\n\n**太字テキスト**')
  ...
  test('WYSIWYG モードではプレビュー切り替えボタンが非表示であること', async ({ page }) => {
    await page.goto('/notes/new')
    await page.click('button:has-text("WYSIWYGモード")')
    await expect(page.locator('button:has-text("プレビューを非表示")')).not.toBeVisible()
    await expect(page.locator('button:has-text("プレビューを表示")')).not.toBeVisible()
  })
})
