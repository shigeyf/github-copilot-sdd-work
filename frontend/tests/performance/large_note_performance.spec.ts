...
test.describe('SC-008: 大規模ノート編集パフォーマンス', () => {
  test('10,000文字のノートの保存が2秒以内に完了すること', async ({ page }) => {
    ...
    await expect(page.locator('input[id="note-title"]')).toHaveValue('大規模テスト')
  ...
  test('10,000文字のノートの編集ページが正常にロードされること', async ({ page }) => {
    const largeContent = generateLargeText(10000)
    ...
    const startTime = Date.now()
    await page.goto(`/notes/${note.id}/edit`)
    await expect(page.locator('input[id="note-title"]')).toHaveValue('ロードテスト')
    const elapsed = Date.now() - startTime
